...
public class TelaCadastroFornecedor extends Tela {
	...
	private RegraCadastroFornecedor regraFornecedor;
	...
	public TelaCadastroFornecedor(TelaPrincipal telaPrincipal) {
		super();
		regraFornecedor = new RegraCadastroFornecedor(this);
	...
	private void inicializarLayoutEEventos() {
		...
		lblCnpj.setBounds(59, 151, 46, 14);
		getContentPane().add(lblCnpj);

		textFieldCnpj = new JTextField();
		textFieldCnpj.setBounds(120, 148, 181, 20);
	...
	protected void incluir() {
		if (isFormularioValido()) {
			...
			if (JOptionPane.OK_OPTION == result) {
				regraFornecedor.incluirFornecedor();
				limpaCampos();
				inicializarComboBoxCodigo();
			}
		} else {
	...
	protected void excluir() {
		regraFornecedor.excluirFornecedor();
		limpaCampos();
		inicializarComboBoxCodigo();
	}
	...
	protected void alterar() {
		if (isFormularioValido()) {
			int result = JOptionPane.showConfirmDialog(null, "Confirmar a alteração?", "Confirmar",
					JOptionPane.OK_CANCEL_OPTION);
			if (JOptionPane.OK_OPTION == result) {
				regraFornecedor.alterarFornecedor();
				limpaCampos();
				inicializarComboBoxCodigo();
	...
	public Fornecedor getFornecedorSelecionado() {
		return ((Fornecedor) comboBoxCodigo.getSelectedItem());
	}
	...
	public void setTextFieldCnpj(String text) {
		this.textFieldCnpj.setText(text);
	}
	...
	public void setSelectedComboBoxCodigo(Fornecedor fornecedor) {
		this.comboBoxCodigo.getModel().setSelectedItem(fornecedor);
	}
